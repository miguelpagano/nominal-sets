...
module Set (A-setoid : DecSetoid ℓ ℓ') where
  ...
  any-cong P Q sub = Any-cong (λ a → sub {a}) id
  ...
  length-minus-sing (x ∷ xs) y (x# ∷ xs#) (here y=x) = ≡-cong (1 +_) (≡-cong length eq)
    where
    y# : All (y ≉_) xs
    y# = ¬Any⇒All¬ xs (¬any-cong (x ≈_) (y ≈_) (λ x₁ → trans (sym y=x) x₁) (All¬⇒¬Any x#))
    eq : xs ≡ (x ∷ xs) ∖[ y ]
    eq with y ≟ x
    ... | yes eq =  ≡-sym (filter-all (¬? ∘ (y ≟_)) y#)
    ... | no ¬eq = contradiction y=x ¬eq
  length-minus-sing (x ∷ xs) y (x# ∷ xs#) (there y∈xs) = begin
    length (x ∷ xs)              ≡⟨ _≡_.refl ⟩
    1 + length xs                ≡⟨ ≡-cong (1 +_) ih ⟩
    1 + length (x ∷ (xs ∖[ y ])) ≡⟨ ≡-cong ((1 +_) ∘ length) (≡-sym eq) ⟩
    1 + length ((x ∷ xs) ∖[ y ]) ∎
    where
  ...
  card-mono {[]} _ _ sub = z≤n
  card-mono {x ∷ xs} {ys} (x# ∷ xs#) ys# sub with x ∈? xs
  ... | yes _ = card-mono xs# ys# (sub ∘ there)
